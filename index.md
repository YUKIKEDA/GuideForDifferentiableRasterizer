...
**技術選定**: DirectX 12 / Windows / C++（Rust でも同一アーキテクチャで実装可能）

**スコープについて**: テクスチャサンプリング・アンチエイリアシング・マルチ解像度・シルエット勾配は、**微分可能レンダリングの本質から外れた付加機能ではない**。画質・勾配の正しさ・実用性のために重要である。Part IX では、まず **ラスタライズ＋頂点属性補間＋解析的勾配** という最小構成に絞り、フルスクラッチで一通り動くところまでを扱う。上記の機能は **今後の拡張** として位置づけ、第 22 章 22.6 で「なぜ重要か」「どの順で足すか」を述べる。

### 22. 技術選定とアーキテクチャ
- 22.1 採用スタックの理由（DirectX 12：Windows ネイティブ・コンピュートシェーダー・HLSL/DXIL）
- 22.2 プロジェクト構成（CMake + vcpkg、Windows SDK、最小依存）
- 22.3 データフロー設計（頂点 → MVP → ラスタライズ → フレームバッファ → 勾配）
- 22.4 CPU/GPU 境界とバッファレイアウト（ID3D12Resource、デスクリプタヒープ、ホスト/デバイス転送）

### 23. CPU フルスクラッチ実装（C++）
- 23.1 MVP 変換の実装（math_util / Eigen の利用）
- 23.2 エッジ関数・重心座標によるラスタライザ本体
- 23.3 深度バッファと可視性判定、属性補間（色・UV・法線）とパースペクティブ補正
- 23.4 逆伝播の完全手実装（自動微分ライブラリに依存しない）

### 24. GPU フルスクラッチ実装（DirectX 12 + C++）
- 24.1 DirectX 12 環境構築（デバイス・コマンドキュー・コンピュートパイプラインステート）
- 24.2 コンピュートシェーダー設計（HLSL → DXIL（DXC）、三角形並列 vs ピクセル並列の選択）
- 24.3 ラスタライズカーネル（エッジ関数・重心座標・深度）とアトミック深度ソート
- 24.4 勾配用コンピュートシェーダー（逆伝播の式をそのまま実装、DL フレームワーク非依存）
- 24.5 ビルド（Windows SDK、DXC）、Windows での実行・検証

### 25. 検証と nvdiffrast との比較
...
| Part IX | 上級者・自作志向 | DirectX 12 + C++（Windows）で CPU（章 23）と GPU（章 24）をフルスクラッチ実装し、検証まで行える（Rust でも同構成で可能） |
